...
context=$(mktemp -d /tmp/mkchad-v1/container-tools-c11/nvim-x86-docker-context.XXXXXX)
trap 'rm -rf -- "$context"' EXIT

container_tools=$("$nvim_dir/bin/resolve_container_tools.sh")

cp -a -- "$nvim_dir/." "$context"
"$nvim_dir/bin/stage_container_tools_package.sh" x86_64 "$context"
"$container_tools" buildx exec --architecture x86_64 -- docker buildx build \
    ...
    --platform linux/x86_64 -f "$context/x86/nvim_container_x86.dockerfile" \
    -t nvim_container_x86:latest --load "$context"
