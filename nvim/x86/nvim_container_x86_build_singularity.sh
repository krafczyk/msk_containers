...
context=$(mktemp -d /tmp/mkchad-v1/container-tools-c11/nvim-x86-sif-context.XXXXXX)
...
"$script_dir/../bin/stage_container_tools_package.sh" x86_64 "$context"
(
    cd "$context"
    "$container_tools" runtime exec --backend "${SINGULARITY##*/}" -- "$SINGULARITY" build --force --fakeroot "$script_dir/nvim_container_x86.sif" nvim_container_x86.def
)
