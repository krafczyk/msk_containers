...
ARG OPENCODE_VERSION=1.18.3-mkchad.3
ARG OPENCODE_REVISION=5ef13270f81cb7ac7bfe11d6c444fdd8ca47467d
...
RUN npm install -g basedpyright \
    ...
    install -m 0755 dist/opencode-linux-arm64/bin/opencode /usr/local/bin/opencode && \
    cd / && \
    rm -rf /nvim/opencode && \
    npm uninstall -g bun && \
