...
| 2026-07-14 | repair pass 1 / mkchad | Resolved `SPOS-AUD-P0-001` and `SPOS-AUD-P1-001`. Managed health reuse now also requires the recorded PID to own the recorded listening socket. Port probing binds and listens; an automatic retry excludes its failed candidate. Isolated headless lifecycle coverage verifies an exact-argv non-listener with a separate healthy responder is refused without signaling it, occupied 4096 falls back, controlled HTTP/non-HTTP listener probing, and failed-candidate exclusion. Startup-race, delayed-concurrent-contender (one persisted high fallback/generation), and reload fixtures also pass. |
| 2026-07-14 | repair pass 1 / opencode.nvim | `SPOS-AUD-P0-002` is blocked by remote availability. The secure stdin-config implementation and its curl argv regression are present locally at exact revision `3238587` and the regression passes there, but `origin/builder/single-opencode-server` remains exactly `ab5eefe` and does not contain that commit. MkChad therefore cannot safely pin the reviewed secure revision without making Lazy fetch an unavailable commit. No push was performed. Authorization to push `3238587` (or provision an equivalent reviewed remote revision), followed by a MkChad pin update and exact-revision verification, is required. |
